...
import pickle
import json
from augment.replace_nl import BertSampler
...
def main():
    ...
    for path, subdirs, files in os.walk(currentpath + '/CACAPO/nl/'):
        for name in files:
    ...
    with open(currentpath + '/Data/NL_AllEntryTemplateInfo_CACAPO.json', 'w') as outfile:
...
def data_augmentation(allentrytemplateinfo, domain):
    ...
    if os.path.isfile(currentpath + '/DonePickle_CACAPO_NL_' + domain + '.pkl'):
        previousdonelist = []
        with open(currentpath + '/DonePickle_CACAPO_NL_' + domain + '.pkl', 'rb') as fr:
            try:
                while True:
    ...
    for entrytemplateidx, entrytemplate in enumerate(allentrytemplateinfo):
        for targettemplatedictidx, targettemplatedict in enumerate(entrytemplate):
            ...
            for idx, token in enumerate(doc):
                #print(token.text, token.lemma_, token.pos_, token.tag_, token.dep_, token.shape_, token.is_alpha, token.is_stop)
                if (token.pos_ == 'NOUN') or (token.pos_ == 'PROPN') or (token.pos_ == 'ADJ') or (token.pos_ == 'ADV'): #or (token.pos_ == 'VERB'):
                    idxlist.append(idx)
            ...
            with open(currentpath + '/DonePickle_CACAPO_NL_' + domain + '.pkl', 'ab') as f:
    ...
    for candlist in candidatesdict:
        ...
        with open(currentpath + '/Predictions/CACAPO/NL_Extended' + candlist + '_' + domain + '_trg.txt', 'wb') as f:
            f.write(bytes(alltrgstring, 'UTF-8'))

        with open(currentpath + '/Predictions/CACAPO/NL_Extended' + candlist + '_' + domain + '_src.txt', 'wb') as f:
...
def collect_dict():
    ...
    with open(currentpath + '/Data/NL_AllEntryTemplateInfo_CACAPO.json', 'r') as infile:
